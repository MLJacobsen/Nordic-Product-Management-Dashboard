...
function AumOverview() {
  ...
  const totalAum = snowflakeAum.reduce((sum, f) => sum + f.aumMillNok, 0);
  const sorted = [...snowflakeAum].sort((a, b) => b.aumMillNok - a.aumMillNok);
    ...
    >
      ...
      <div className="bg-gradient-to-r from-primary-50 to-primary-100 rounded-xl p-4 border border-primary-200">
        ...
        <p className="text-[10px] text-neutral-400 mt-1">
          {snowflakeAum.length} funds · {noShareClassCount} share classes · As of {aumDataDate}
        </p>
      </div>
          ...
          {sorted.map((fund, i) => (
            <div key={fund.fundId} className="flex items-center justify-between text-[11px] py-0.5 border-b border-neutral-50">
              ...
              <span className="font-medium text-neutral-700 shrink-0">
                {fund.aumMillNok.toLocaleString('en-GB')} mn
              </span>
            </div>
          ))}
        </motion.div>
